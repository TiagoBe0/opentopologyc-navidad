#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# opentopologyc/core/alpha_shape_filter.py
...
def create_ghost_layers(positions, box_bounds, lattice_param, num_layers=2):
    ...
    for region_key, atoms in atoms_by_region.items():
        ...
        for orig_idx, orig_pos in atoms:
            ...
            # Generar shifts según número de dimensiones
            if num_dims == 1:
                # CARA: solo un shift
...
class AlphaShapeWithGhosts:
    ...
    def _build_mesh(self, delaunay, surface_facets):
        ...
        # FILTRAR: mantener solo índices de átomos REALES
        ...
        # Guardar índices de átomos superficiales
        ...
        print(f"    - Caras válidas: {len(faces)}")
    ...
    def get_surface_atoms_indices(self):
        """Obtener índices de átomos superficiales"""
...
class LAMMPSDumpParser:
    ...
    def write(filename, data, filtered_atom_ids):
        """Escribe un archivo dump LAMMPS filtrado"""
...
def filter_surface_atoms(input_dump, output_dump, probe_radius=2.0,
                         lattice_param=None, num_ghost_layers=2,
                         smoothing=0):
    """
    Filtra átomos superficiales de un dump usando Alpha Shape con Ghost Particles

    Args:
        input_dump: Ruta al dump de entrada
        output_dump: Ruta al dump de salida
        probe_radius: Radio de la esfera de prueba
    ...
    """
    # Leer
    print(f"\n[CARGA] Leyendo: {input_dump}")
    data = LAMMPSDumpParser.read(input_dump)
    ...
    # Exportar
    print(f"\n[EXPORTACIÓN] Escribiendo: {output_dump}")
    filtered_ids = sorted([data['atom_ids_ordered'][idx] for idx in surface_atoms])
    LAMMPSDumpParser.write(output_dump, data, filtered_ids)

    result = {
        'input_atoms': len(data['atoms']),
        'output_atoms': len(filtered_ids),
        'removed_atoms': len(data['atoms']) - len(filtered_ids),
        'surface_area': constructor.surface_area,
        'output_file': output_dump
    }

    print(f"\n{'='*80}")
    print(f"✓ COMPLETADO")
    print(f"{'='*80}")
    print(f"  Entrada:  {result['input_atoms']} átomos")
    print(f"  Salida:   {result['output_atoms']} átomos")
    print(f"  Removidos: {result['removed_atoms']}")
    print(f"  Área: {result['surface_area']:.4f}")
    print(f"\n✓ Guardado: {output_dump}\n")
